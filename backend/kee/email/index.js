...
function validateSyntax(email) {
    ...
    if (!email.includes("@"))
        return { valid: false, details: "Missing @ symbol" };
    const [local, domain] = email.split("@");
    if (local.length === 0)
        return { valid: false, details: "Empty local part" };
    if (local.length > 64)
        return { valid: false, details: "Local part exceeds 64 characters" };
    if (!domain || !domain.includes("."))
        return { valid: false, details: "Invalid domain" };
    if (domain.length > 255)
        return { valid: false, details: "Domain exceeds 255 characters" };
    if (!regex.test(email))
        return { valid: false, details: "Invalid email format" };
...
async function verifyEmail(email) {
    ...
    if (totalScore >= 70)
        status = "valid";
    else if (disposable.disposable)
        status = "disposable";
    else if (totalScore < 30)
        status = "invalid";
    else if (catchAll.catchAll)
        status = "catch-all";
    return {
        email,
