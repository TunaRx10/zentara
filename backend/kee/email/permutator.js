...
exports.verifyPermutations = verifyPermutations;
exports.findLikelyEmails = findLikelyEmails;
 ...
 */
async function verifyPermutations(emails) {
    if (!emails.length)
        return [];
 ...
 */
async function findLikelyEmails(firstName, lastName, domain) {
...
async function checkMxRecords(domain) {
    try {
        ...
        let provider = "Custom";
        if (records.includes("google") || records.includes("gmail"))
            provider = "Google Workspace";
        else if (records.includes("outlook") || records.includes("microsoft"))
            provider = "Microsoft 365";
        else if (records.includes("proton"))
            provider = "ProtonMail";
        else if (records.includes("zoho"))
            provider = "Zoho";
        else if (records.includes("amazon") || records.includes("ses"))
            provider = "Amazon SES";
        else if (records.includes("yahoo"))
            provider = "Yahoo";
        else if (records.includes("icloud") || records.includes("apple"))
            provider = "iCloud";
