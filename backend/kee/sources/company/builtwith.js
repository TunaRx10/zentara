...
exports.BuiltWithSource = void 0;
// BuiltWith / WhatRuns — Tech Stack Detection (FREE via web scraping)
const base_1 = require("../base");
class BuiltWithSource extends base_1.BaseSource {
    ...
    async analyzeSite(domain) {
        ...
        try {
            ...
            if (server)
                entries.push({ name: server, category: "Web Server" });
            if (poweredBy)
                entries.push({ name: poweredBy, category: "Backend Framework" });
            if (via)
                entries.push({ name: `Via: ${via}`, category: "CDN" });
            // Check for common CDN/hosting headers
            const cfRay = res.headers.get("cf-ray");
            if (cfRay)
                entries.push({ name: "Cloudflare", category: "CDN" });
            const xAmzCfId = res.headers.get("x-amz-cf-id");
            if (xAmzCfId)
                entries.push({ name: "AWS CloudFront", category: "CDN" });
            const xVercelId = res.headers.get("x-vercel-id");
            if (xVercelId)
                entries.push({ name: "Vercel", category: "Hosting" });
            const xNetlify = res.headers.get("x-nf-request-id");
            if (xNetlify)
                entries.push({ name: "Netlify", category: "Hosting" });
            // Analyze HTML for meta/script clues
            const html = await res.text();
            // Meta generator tag
            const generatorMatch = html.match(/<meta[^>]*name=["']generator["'][^>]*content=["']([^"']+)["']/i);
            if (generatorMatch)
                entries.push({ name: generatorMatch[1], category: "CMS" });
            // Script sources
            ...
            while ((scriptMatch = scriptSrcRegex.exec(html)) !== null) {
                ...
                if (src.includes("gtag") || src.includes("analytics"))
                    entries.push({ name: "Google Analytics", category: "Analytics" });
                if (src.includes("googletagmanager"))
                    entries.push({ name: "Google Tag Manager", category: "Tag Manager" });
                if (src.includes("hotjar"))
                    entries.push({ name: "Hotjar", category: "Analytics" });
                if (src.includes("segment.com") || src.includes("segment.io"))
                    entries.push({ name: "Segment", category: "Analytics" });
                if (src.includes("mixpanel"))
                    entries.push({ name: "Mixpanel", category: "Analytics" });
                if (src.includes("amplitude"))
                    entries.push({ name: "Amplitude", category: "Analytics" });
                if (src.includes("intercom"))
                    entries.push({ name: "Intercom", category: "Customer Support" });
                if (src.includes("zendesk") || src.includes("zdassets"))
                    entries.push({ name: "Zendesk", category: "Customer Support" });
                if (src.includes("drift"))
                    entries.push({ name: "Drift", category: "Customer Support" });
                if (src.includes("crisp"))
                    entries.push({ name: "Crisp", category: "Customer Support" });
                if (src.includes("tawk.to"))
                    entries.push({ name: "Tawk.to", category: "Customer Support" });
                if (src.includes("hubspot"))
                    entries.push({ name: "HubSpot", category: "Marketing" });
                if (src.includes("mailchimp"))
                    entries.push({ name: "Mailchimp", category: "Email Marketing" });
                if (src.includes("stripe"))
                    entries.push({ name: "Stripe", category: "Payment" });
                if (src.includes("paypal"))
                    entries.push({ name: "PayPal", category: "Payment" });
                if (src.includes("sentry"))
                    entries.push({ name: "Sentry", category: "Error Tracking" });
                if (src.includes("newrelic") || src.includes("nr-data"))
                    entries.push({ name: "New Relic", category: "Performance Monitoring" });
                if (src.includes("datadog"))
                    entries.push({ name: "Datadog", category: "Performance Monitoring" });
            }
            ...
            if (html.includes("__NEXT_DATA__"))
                entries.push({ name: "Next.js", category: "JavaScript Frameworks" });
            if (html.includes("__NUXT__") || html.includes("nuxt"))
                entries.push({ name: "Nuxt.js", category: "JavaScript Frameworks" });
            if (html.includes('id="__vite_ssr"') || html.includes("vite"))
                entries.push({ name: "Vite", category: "Build Tool" });
            if (html.includes("wp-content") || html.includes("wp-includes"))
                entries.push({ name: "WordPress", category: "CMS" });
            if (html.includes("Shopify"))
                entries.push({ name: "Shopify", category: "E-Commerce" });
            if (html.includes("react-root") || html.includes("__react"))
                entries.push({ name: "React", category: "JavaScript Frameworks" });
            if (html.includes("ng-version") || html.includes("ng-app"))
                entries.push({ name: "Angular", category: "JavaScript Frameworks" });
            if (html.includes("data-v-") || html.includes("vue"))
                entries.push({ name: "Vue.js", category: "JavaScript Frameworks" });
            if (html.includes("svelte"))
                entries.push({ name: "Svelte", category: "JavaScript Frameworks" });
            if (html.includes("tailwindcss") || html.includes("tailwind"))
                entries.push({ name: "Tailwind CSS", category: "CSS Framework" });
            if (html.includes("bootstrap"))
                entries.push({ name: "Bootstrap", category: "CSS Framework" });
            // Check for SSL/HTTPS
...
exports.BuiltWithSource = BuiltWithSource;
exports.default = new BuiltWithSource();
