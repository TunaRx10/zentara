...
exports.OpenStreetMapSource = void 0;
...
class OpenStreetMapSource extends base_1.BaseSource {
    name = "OpenStreetMap";
    id = "openstreetmap";
    category = "local";
    ...
    lookupBusinessType(term) {
        // Direct match
        if (BUSINESS_TYPE_MAP[term])
            return BUSINESS_TYPE_MAP[term];
        // Try removing trailing 's' for plurals
        const singular = term.replace(/s$/, "");
        if (BUSINESS_TYPE_MAP[singular])
            return BUSINESS_TYPE_MAP[singular];
     ...
     */
    buildOverpassQuery(parsed, bbox) {
...
out center body;`;
     ...
     */
    elementToLead(element, parsed, locationName) {
        ...
        if (tags.brand)
            metadata.brand = tags.brand;
        if (tags.operator)
            metadata.operator = tags.operator;
        if (tags.description)
    ...
    async searchWithGeo(parsed, geo, options) {
    ...
    async getCompany(domain) {
        ...
        const elements = await this.queryOverpass(`[out:json][timeout:10];
(
...
exports.OpenStreetMapSource = OpenStreetMapSource;
exports.default = new OpenStreetMapSource();
